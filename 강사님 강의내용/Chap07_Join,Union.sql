...
두가지 방법으로 과일 가게의 일자 별 마진 금액을 산출하세요 . 
  ...
  .1 : UNION 을 사용하여 마진금액 표기
  .2 : UNION 을 사용하지 않고 마진 금액 표기
...
-- 1) UNION 을 그대로 쓰는 방법. 1
SELECT AA.DATE,
	   SUM(AA.INOUTPRICE)
  FROM (SELECT A.DATE                   AS DATE
	          ,SUM(A.AMOUNT * C.PRICE)	AS INOUTPRICE
	      FROM T_SalesList A JOIN T_Custmer B
	   						ON A.CUST_ID = B.CUST_ID
	   					  JOIN T_Fruit C 
	   					    ON A.FURIT = C.FRUIT
