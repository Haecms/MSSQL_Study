/*****************************************************************************
1. 쿼리 문에서 변수를 지정하고 사용하는 방법 

-- DECLARE @[변수명][데이터타입](크기) */

-- 변수를 선언 할 때는 반드시 @ 가 앞에 있어야 한다. 
...
SELECT @LS_VALUE,@LI_VALUE AS III
...
2. 분기문 (IF)
 ...
 DECLARE @LS_MESSGE  VARCHAR(30), -- 메세지 가 담길 변수. 
...
IF (@LI_INVALUE > 900)
BEGIN 
	SET @LS_MESSGE = '900 보다 큽니다.';
END 
ELSE IF (@LI_INVALUE <= 900 AND @LI_INVALUE > 400)
BEGIN
	SET @LS_MESSGE = '400 보다크고 900 이하 입니다.';
END
ELSE
BEGIN  
	SET @LS_MESSGE = '400 이하입니다.'
END
SELECT @LS_MESSGE;
...
3. 분기문 (CASE WHEN THEN END)
  ...
  1. CASE [비교대상] WHEN [조건] THEN [결과] END
    - 고정된 비교 대상과 다수의 조건에 따른 결과를 표현할 경우.
  
  2. CASE WHEN [비교대상 , 비교연산자, 조건] THEN [결과] END
    - 다수의 비교 대상과 다수의 조건에 따른 결과를 표현할 경우. */

-- 1) 조회부 SELECT 에서 사용되는 CASE WHEN 문
...
-- 1-2) CASE WHEN [비교대상 , 비교연산자, 조건] THEN [결과] END
		...
		SELECT ITEMCODE,
			   ...
			   CASE WHEN STOCKQTY <= 0 THEN '재고없음'
					...
					ELSE '재고초과' END STOCKSTATUS
		  FROM TB_StockMM

-- 2) 조건부 에 사용되는 CASE WHEN 
-- 2-1
...
SET @LI_STOCKQTY = 500;
   ...
   SELECT * 
     FROM TB_StockMM
	WHERE MATLOTNO = CASE @LI_STOCKQTY WHEN 1700 THEN 'LTROH1459097100001'
									   ELSE 'LTROH1438534870001' END
...
/********************************************************************************************
 ...
 /* TB_StockMMrec : 자재 입출 이력 테이블.
	...
	자재 입출 이력 테이블에서 
	PLANTCODE, INOUTDATE, INOUTSEQ, ITEMCODE 컬럼의 데이터 를 검색하고 
	ITEMCODE 가 NULL 일 경우 X 로 표현하세요. */
