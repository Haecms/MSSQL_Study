...
BEGIN
	SET @LS_MESSAGE = '400보다 크고 900이하 입니다.';
END
ELSE
BEGIN
...
-- 1-2) CASE WHEN [비교대상, 비교연산자, 조건] THEN [결과] END			- 비교해서 데이터를 표현할 때
		
		SELECT  ITEMCODE,
				STOCKQTY,
				CASE WHEN STOCKQTY <= 0 THEN '재고 없음'
					 WHEN STOCKQTY > 0 AND STOCKQTY <=1000 THEN '안전재고'
					 ELSE '재고초과' END STOCKSTATUS
		  FROM TB_StockMM

-- 2) 조건부에 사용되는 CASE WHEN										- 조회되는 결과 자체를 다르게 바꿀 수 있음
-- 2-1
...
SELECT PLANTCODE,
       INOUTDATE,
	   INOUTSEQ,
	   ISNULL(ITEMCODE, 'X') AS ITCD  --아이템코드에 널이 있다면 X로 가공처리해서 나타내라
  FROM TB_StockMMrec
...
SELECT ITEMCODE,
	   ...
	   LOCCODE
  FROM TB_ItemMaster
 WHERE ISNULL(LOCCODE,'') LIKE '%%' -- 조회가 안 되는데 LIKE '%%'로 조회가능하게 함
